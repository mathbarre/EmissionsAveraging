...
from CH4Avg.utils.dev_unbalanced_gpu import (barycenter_unbalanced_sinkhorn2D_gpu,
...
G2D = barycenter_unbalanced_sinkhorn2D_gpu(
    Tot_gpu, Cx_gpu, Cy_gpu, reg, reg_m, weights=cp.array(Tot.sum(axis=0).sum(axis=0)/Tot.sum()),
    numItermax=400, stopThr=1e-4, verbose=True,
    log=True, logspace=True, reg_K=1e-16)
G2Dw = barycenter_unbalanced_sinkhorn2D_wind_gpu(
    Tot_gpu, Cxs_gpu, Cys_gpu, reg, reg_m, weights=cp.array(Tot.sum(axis=0).sum(axis=0)/Tot.sum()),
...
ax1 = plt.subplot(132)
plt.imshow(G2D[0].get(), cmap=cm)
plt.title("2D Wasserstein")
ax1 = plt.subplot(133)
plt.imshow(G2Dw[0].get(), cmap=cm)
